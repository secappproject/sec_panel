...
class HomeScreenState extends State<HomeScreen> with TickerProviderStateMixin {
  ...
  bool _isHoveringWiring = false;
  String wiringDisplayStatus = "Open";
  ...
  List<String> selectedWiring = [];
  List<String> selectedWiringVendors = [];
  ...
  List<String> selectedProductTypes = ["All"];
  ...
  Future<void> loadInitialData() async {
    ...
    try {
      final rawData = await DatabaseHelper.instance.getAllPanelsForDisplay(
      ...
      debugPrint("Gagal parsing data: $e");
      if (mounted) {
  ...
  void _prepareChartData() {
    ...
    final targetPanels = panelsToDisplay
        .where((p) => p.targetDelivery != null)
        .toList();
    _totalTargetQty = targetPanels.length;

    _totalClosedQty = targetPanels
        .where((p) => p.panel.isClosed == true)
        .length;

    _achievementRate = _totalTargetQty > 0
        ? (_totalClosedQty / _totalTargetQty) * 100
        : 0.0;
    ...
    _panelWipChartData = _calculateTargetDeliveryByTime(
      panelsToDisplay,
      (data) => [
        data.panelVendorName.isNotEmpty ? data.panelVendorName : "No Vendor",
      ],
    ...
    _busbarWipChartData = _calculateBusbarTargetDeliveryByTime(
      panelsToDisplay,
      (data) {
        if (data.busbarVendorNames.isNotEmpty) {
          return data.busbarVendorNames
    ...
    _projectWipChartData = _calculateTargetDeliveryByProject(
      panelsToDisplay,
      view: _projectChartView,
      year: _selectedYear,
      years: _selectedYears,
    ...
    showModalBottomSheet(
      ...
      builder: (_) => PanelFilterBottomSheet(
        ...
        onPositionsChanged: (value) =>
            setState(() => selectedPositions = value),
        ...
        selectedWiring: selectedWiring,
        selectedWiringVendors: selectedWiringVendors,
        ...
        onWiringChanged: (value) => setState(() => selectedWiring = value),
        onWiringVendorsChanged: (value) =>
            setState(() => selectedWiringVendors = value),
        ...
        onSrStatusChanged: (value) => setState(() => selectedSrStatus = value),
  ...
  void _openTransferPanelBottomSheet(PanelDisplayData panelData) {
    showModalBottomSheet(
      ...
      builder: (_) => TransferPanelBottomSheet(
        ...
        panelData: panelData,
        onSuccess: (updatedPanelData) async {
          Navigator.of(context).pop();
          await loadInitialData();
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text("Status panel berhasil diperbarui"),
  ...
  Future<void> _openEditPanelBottomSheet(PanelDisplayData dataToEdit) async {
    // Reload data dulu supaya ambil versi terbaru
    await loadInitialData();

    // Ambil ulang data berdasarkan noPp dari list yang benar
    final refreshedData = _allPanelsData.firstWhere(
      (p) => p.panel.noPp == dataToEdit.panel.noPp,
    );
    ...
    await showModalBottomSheet(
      ...
      builder: (_) => EditPanelBottomSheet(
        panelData: refreshedData, // ✅ pakai data terbaru
        currentCompany: widget.currentCompany,
        k3Vendors: _allK3Vendors,
        onSave: (updatedPanel) async {
          await loadInitialData();
          if (mounted) setState(() {});
        },
        onDelete: () async {
          Navigator.of(context).pop();
          await DatabaseHelper.instance.deletePanel(
            dataToEdit.panel.noPp ?? '',
          );
          await loadInitialData();
          if (mounted) {
            setState(() {});
  ...
  void _openEditStatusBottomSheet(PanelDisplayData dataToEdit) {
    showModalBottomSheet(
      ...
      builder: (_) => EditStatusBottomSheet(
        duration: _formatDuration(dataToEdit.productionDate),
        startDate: dataToEdit.productionDate,
  ...
  PanelFilterStatus _getPanelFilterStatus(Panel panel) {
    ...
    if (progress >= 50 && progress < 75)
      return PanelFilterStatus.progressOrange;
  ...
  List<PanelDisplayData> get _panelsAfterPrimaryFilters {
    return _allPanelsData.where((data) {
      ...
      final bool matchProductType =
          selectedProductTypes.contains("All") ||
      ...
      if (selectedPositions.isEmpty) {
        ...
        if (selectedPositions.contains("Warehouse")) {
          matchPosition =
              (currentPosition == 'Warehouse' ||
              currentPosition == 'VendorWarehouse');
        } else {
  ...
  List<PanelDisplayData> get filteredPanelsForDisplay {
    ...
    switch (_tabController.index) {
      ...
      case 2:
        tabFilteredPanels = tabFilteredPanels.where((data) {
          return data.panel.targetDelivery != null;
        }).toList();
        break;
      case 3:
        tabFilteredPanels = tabFilteredPanels
    ...
    tabFilteredPanels.sort((a, b) {
      ...
      switch (sort) {
        ...
        // PERBAIKAN: Ganti .startDate menjadi .productionDate
        case SortOption.durationAsc:
          return (a.productionDate ?? DateTime(2200)).compareTo(
            b.productionDate ?? DateTime(2200),
          );
        case SortOption.durationDesc:
          return (b.productionDate ?? DateTime(1900)).compareTo(
            a.productionDate ?? DateTime(1900),
          );
        ...
        default:
          // Default sorting juga harus menggunakan productionDate
          return (b.productionDate ?? DateTime(1900)).compareTo(
            a.productionDate ?? DateTime(1900),
          );
      }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          buildSection('In Production', '7 Panel in Total', [
            buildTag('1 di LV1'),
            buildTag('1 di LV2'),
            buildTag('1 di LV3'),
            buildTag('1 di LV4'),
          ...
          buildSection('Out Production', '7 Panel in Total', [
            buildTag('1 di LV1'),
            buildTag('1 di LV2'),
            buildTag('1 di LV3'),
            buildTag('1 di LV4'),
  ...
  Widget _buildContentView() {
    ...
    final targetDeliveryCount = baseFilteredList.where((data) {
      return data.panel.targetDelivery != null;
    }).length;
    ...
    final tabBarWidget = TabBar(
      ...
      tabs: [
        ...
        Tab(text: "Target Delivery ($targetDeliveryCount)"),
      ...
      children: [
        Padding(
          ...
          child: Column(
            ...
            children: [
              const Text(
                "Alignment Panel Busbar Komponen & Wiring",
                style: TextStyle(
                ...
                children: [
                  ...
                  InkWell(
                    ...
                    child: Container(
                      ...
                      child: _isChartView
                          ? Image.asset('assets/images/panel.png', height: 20)
        ...
        return GridView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            return PanelProgressCard(
              ...
              duration: _formatDuration(data.productionDate),
              progress: (panel.percentProgress ?? 0) / 100.0,
              startDate: data.productionDate,
              ...
              onPanelUpdated: (updatedPanel) {
                setState(() {
                  final index = _allPanelsData.indexWhere(
                    (p) => p.panel.noPp == updatedPanel.panel.noPp,
                  );
                  if (index != -1) {
                    _allPanelsData[index] = updatedPanel;
                    _allPanelsData = List<PanelDisplayData>.from(
                      _allPanelsData,
                    );
                  }
                });
      ...
      sliver: SliverList.separated(
        ...
        itemBuilder: (context, index) {
          ...
          return PanelProgressCard(
            ...
            statusPalet: panel.statusPalet ?? "",
            statusCorepart: panel.statusCorepart ?? "",
            statusWiring: panel.statusWiring ?? "",
            ppNumber: panel.noPp ?? "",
            wbsNumber: panel.noWbs ?? "",
            project: panel.project ?? "",
            onEdit: () {
    ...
    return Container(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          final isSelected = selectedProductTypes.contains(type);
          return GestureDetector(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                color: isSelected ? AppColors.schneiderGreen : Colors.white,
  ...
  Map<String, dynamic> _calculateBusbarDeliveryByTime(
    ...
    switch (view) {
      ...
      case ChartTimeView.yearly:
        ...
        displayRange = DateTimeRange(
          start: DateTime(years.isEmpty ? year : years.first, 1, 1),
          end: DateTime(years.isEmpty ? year : years.last, 12, 31),
        );
        break;
    }
    ...
    final relevantPanels = panels.where(
      (data) =>
          (data.panel.statusBusbarPcc ?? '') == 'Close' &&
          data.targetDelivery != null,
    );

    for (var data in relevantPanels) {
      final dateForChart = data.targetDelivery!;

      bool isInRange = view == ChartTimeView.yearly
          ? years.contains(dateForChart.year)
          : (!dateForChart.isBefore(displayRange.start) &&
                !dateForChart.isAfter(
                  displayRange.end.add(const Duration(seconds: 1)),
                ));
      ...
      for (var vendor in getVendors(data)) {
    ...
    return {'data': counts, 'vendors': allPossibleVendors..sort()};
  }

  Map<String, dynamic> _calculateBusbarTargetDeliveryByTime(
    List<PanelDisplayData> panels,
    List<String> Function(PanelDisplayData) getVendors, {
    required ChartTimeView view,
    required List<String> allPossibleVendors,
    ...
    final relevantPanels = panels.where((data) => data.targetDelivery != null);
  ...
  Map<String, dynamic> _calculateTargetDeliveryByTime(
    List<PanelDisplayData> panels,
    List<String> Function(PanelDisplayData) getVendors, {
    required ChartTimeView view,
    required List<String> allPossibleVendors,
    ...
    for (var data in relevantPanels) {
      ...
      if (view == ChartTimeView.yearly) {
        if (years.contains(dateForChart.year)) isInDateRange = true;
      } else {
    ...
    if (view == ChartTimeView.daily) {
      for (int i = 0; i <= displayRange.duration.inDays; i++) {
        allKeys.add(
          keyFormat.format(displayRange.start.add(Duration(days: i))),
        );
      }
      ...
      if (quartile == null) {
        ...
        for (int i = 0; i < 3; i++)
          allKeys.add(
            keyFormat.format(DateTime(year, (quartile - 1) * 3 + 1 + i)),
          );
      }
    } else {
      for (final yearValue in years) allKeys.add(yearValue.toString());
    }

    for (final key in allKeys) counts.putIfAbsent(key, () => {});

    final sortedKeys = counts.keys.toList()
      ..sort((a, b) {
        try {
          if (view == ChartTimeView.monthly && quartile == null) {
            return DateFormat('MMM', 'id_ID')
                .parse(a)
                .month
                .compareTo(DateFormat('MMM', 'id_ID').parse(b).month);
          }
    ...
    return {
      'data': {for (var k in sortedKeys) k: counts[k]!},
      'vendors': allPossibleVendors..sort(),
    };
  }
    ...
    final relevantPanels = panels.where((data) {
      ...
      return !data.panel.closedDate!.isBefore(displayRange.start) &&
          !data.panel.closedDate!.isAfter(
            displayRange.end.add(const Duration(days: 1)),
          );
    });
  ...
  Map<String, dynamic> _calculateTargetDeliveryByProject(
    List<PanelDisplayData> panels, {
    required ChartTimeView view,
    required int year,
    required List<int> years,
    ...
    for (var data in relevantPanels) {
      final dateForChart = data.productionDate ?? DateTime.now();
              ...
              children: [
                Expanded(
                  child: _GroupedBarChartCard(
                    ...
                    onToggle: (newView) =>
                        setState(() => _panelChartView = newView),
                  ),
                ),
                ...
                Expanded(
                  child: _GroupedBarChartCard(
                    ...
                    onToggle: (newView) =>
                        setState(() => _busbarChartView = newView),
                  ),
                ),
              ],
            ),
  ...
  Widget _buildChartView() {
    ...
    return Column(
      children: [
        // 1. AREA GRAFIK (Sekarang di Atas)
        Expanded(
          child: DefaultTabController(
            ...
            child: Scaffold(
              ...
              body: TabBarView(
                controller: _chartTabController,
                children: [_buildVendorChartView(), _buildProjectChartView()],
              ),
            ),
          ),
        ),

        // 2. KARTU ACHIEVEMENT (Sekarang di Paling Bawah)
        Container(
          width: double.infinity,
          padding: const EdgeInsets.all(16),
          // Margin bottom disesuaikan agar tidak terlalu mepet dengan navigasi bawah
          ...
          child: Column(
            children: [
              const Text(
                "Achievement Delivery vs Target",
                style: TextStyle(
                  fontSize: 12,
              ...
              Row(
                ...
                children: [
                  ...
                  Text(
                    "($_totalClosedQty / $_totalTargetQty Panels)",
                    style: const TextStyle(
                      fontSize: 14,
                      color: AppColors.black,
                      fontFamily: 'Lexend',
  ...
  Widget _buildChartFilterDropdowns({required ChartTimeView currentView}) {
      ...
      builder: (context) {
        return StatefulBuilder(
          builder: (BuildContext context, StateSetter setSheetState) {
            final bool isAllSelected = Set.from(
              tempSelectedYears,
            ).containsAll(availableYears);
  ...
  Widget _buildBarChartItself({
    ...
    final List<String> seriesNames =
        (chartData['vendors'] as List<String>? ??
    ...
    final double widthPerGroup = seriesNames.isEmpty
        ? barWidth
        : (seriesNames.length * barWidth) +
              ((seriesNames.length - 1) * barsSpace);
...
class _GroupedBarChartCardState extends State<_GroupedBarChartCard> {
  ...
  @override
  Widget build(BuildContext context) {
    final List<String> closedSeries =
        (widget.closedChartData['vendors'] as List<String>? ??
        widget.closedChartData['projects'] as List<String>? ??
        []);
    final List<String> wipSeries =
        (widget.wipChartData['vendors'] as List<String>? ??
        widget.wipChartData['projects'] as List<String>? ??
        []);
    final List<String> seriesNames = {...closedSeries, ...wipSeries}.toList()
      ..sort();
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 24),
          Text(
            "Jumlah ${widget.itemType == "by Project" ? "Panel" : widget.itemType} Target Delivery",
            style: const TextStyle(
              fontSize: 14,
              fontWeight: FontWeight.w400,
              color: AppColors.black,
...
class _ScrollableBarChartState extends State<_ScrollableBarChart> {
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      ...
      child:
          widget.data.isEmpty ||
              widget.data.values.every((map) => map.values.every((v) => v == 0))
          ? const Center(
              child: Text(
          ...
          : LayoutBuilder(
              builder: (context, constraints) {
                final double calculatedWidth =
                    (widget.data.keys.length * widget.widthPerGroup) +
                    ((widget.data.keys.length - 1) * widget.groupsSpace);
                      ...
                      child: SizedBox(
                        ...
                        child: BarChart(
                          BarChartData(
                                ...
                                sideTitles: SideTitles(
                                  ...
                                  getTitlesWidget:
                                      (double value, TitleMeta meta) {
                                        ...
                                        if (index >= widget.data.keys.length)
                                          return const SizedBox.shrink();
                                        final key = widget.data.keys.elementAt(
                                          index,
                                        );
                                        final title =
                                            widget.currentView ==
                                ...
                                sideTitles: SideTitles(
                                  ...
                                  getTitlesWidget:
                                      (double value, TitleMeta meta) {
                                        ...
                                        if (index >= widget.data.keys.length)
                                          return const SizedBox.shrink();
                                        final groupKey = widget.data.keys
                                            .elementAt(index);
                                        final total = widget
                                            .data[groupKey]!
                                            .values
                            ...
                            barGroups: List.generate(widget.data.keys.length, (
                              index,
                            ) {
                              ...
                              return BarChartGroupData(
                                ...
                                barRods: List.generate(
                                  ...
                                  (seriesIndex) {
                                    final seriesName =
                                        widget.seriesNames[seriesIndex];
                                    final count =
                                        seriesCounts[seriesName]?.toDouble() ??
                                        0;
                                    return BarChartRodData(
                                      toY: count,
                                      color:
                                          widget.colorMap[seriesName] ??
                                          Colors.grey,
