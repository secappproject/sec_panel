...
class _CompanyFormBottomSheetState extends State<_CompanyFormBottomSheet> {
  ...
  Future<void> _loadCompaniesData() async {
    ...
    if (mounted) {
      setState(() {
        _companiesData = data.map((e) => e as Map<String, dynamic>).toList();
    ...
    if (!_isEditing) {
      ...
      if (isTaken) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text("Username ini sudah digunakan."),
              backgroundColor: AppColors.red,
            ),
          );
  ...
  Future<void> _showCompanyEditSheet(Map<String, dynamic> companyData) async {
    final companyToEdit = Company(
      ...
      role: AppRole.values.firstWhere((e) => e.name == companyData['role']),
    );
...
class _CompanyEditForm extends StatefulWidget {
  ...
  const _CompanyEditForm({required this.company, required this.onSave});
  ...
  Future<void> _saveCompany() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Detail perusahaan berhasil diperbarui'),
            backgroundColor: AppColors.schneiderGreen,
          ),
        );
        widget.onSave();
    ...
    return Column(
      ...
      children: [
        ...
        Wrap(
          spacing: 8,
          runSpacing: 12,
          children: AppRole.values.where((role) => role != AppRole.admin).map((
            role,
          ) {
            return _buildOptionButton(
              label: role.name[0].toUpperCase() + role.name.substring(1),
